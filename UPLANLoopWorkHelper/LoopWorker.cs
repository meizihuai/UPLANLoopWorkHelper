...
namespace UPLANLoopWorkHelper
{
    public class LoopWorker
    {
        ...
        public static void Start()
        {
            ...
            Program.Log("===========UPLAN Server启动，开启循环工作线程===========", TAG);
            foreach (Thread t in threads)
            {
                ...
                {
                    t.Start();
                    Program.Log($"   =>线程[{threadName}]已启动", TAG);
                }
                catch (Exception e)
                {
                    Program.Log($"   =><error>线程[{threadName}]启动失败！！！" + e.ToString(), TAG);
                }
            }
        }
        ...
        public static void Stop()
        {
            ...
            Program.Log("===========UPLAN Server关闭，关闭所有线程===========", TAG);
            foreach (Thread t in threads)
            {
                try
                {
